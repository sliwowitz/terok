...
!!! note "What the gate does and does not guarantee"
    ...
    **not** technically prevent the agent from adding other remotes or
    ...
    - **Per-project SSH keys** — generated by terok, only useful if
      the user deliberately registers them as deploy keys on the
      upstream remote. Without registration, the agent has no more
      access than an anonymous user.
...
!!! tip "Defence in depth"
    ...
    gate (push destination control) + shield (egress firewall) + deploy
    key discipline (no upstream write access unless deliberately granted).
